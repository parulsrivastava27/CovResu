...
# LinkedIn export helper imports
import zipfile
import csv
...
def parse_linkedin_export_file(uploaded_file):
    """
    Accepts an uploaded file-like object from Streamlit (zip or csv),
    returns a dict with parsed profile fields (best-effort).
    """
    parsed = {
        'name': '',
        'email': '',
    ...
    try:
        ...
        if fname.endswith('.zip'):
            ...
            if not csv_names:
                # try json files or plain text fallback
                return parsed
            # choose first CSV that looks likely (profile, basic, or first)
            chosen = None
            for candidate in csv_names:
                if 'profile' in candidate.lower() or 'basic' in candidate.lower() or 'contact' in candidate.lower():
                    chosen = candidate
                    break
            if not chosen:
                chosen = csv_names[0]
            raw = z.read(chosen).decode('utf-8', errors='ignore')
        ...
        # helper to get multiple possible headers
        def get_any(d, keys):
            for k in keys:
        ...
        first = get_any(row, ['First Name','FirstName','First_Name','firstname','first name'])
        last = get_any(row, ['Last Name','LastName','Last_Name','lastname','last name'])
        parsed['name'] = (first + ' ' + last).strip() if (first or last) else get_any(row, ['Full Name','FullName','Name','name'])

        parsed['current_role'] = get_any(row, ['Headline','headline','Job Title','Headline/Current','Position'])
        parsed['summary'] = get_any(row, ['Summary','About','profile summary','profile'])
        parsed['email'] = get_any(row, ['Email','E-mail Address','Email Address','email address','email'])
        parsed['phone'] = get_any(row, ['Phone','Phone Number','Contact','Telephone'])

        # heuristics to extract skills: split headline or summary on pipes/commas
        skills_src = parsed['current_role'] or parsed['summary']
        skills = []
        if skills_src:
            for token in re.split(r'[|,/;•\n]', skills_src):
                t = token.strip()
                if 2 < len(t) < 40 and len(t.split()) <= 6:
                    skills.append(t)
        parsed['skills'] = []
        seen = set()
        for s in skills:
            ...
            if low not in seen:
                ...
                seen.add(low)
            if len(parsed['skills']) >= 30:
                break

        # Experience parsing: detect if CSV looks like positions list
        title_keys = [k for k in row.keys() if any(x in k.lower() for x in ['title','position','job'])]
        company_keys = [k for k in row.keys() if any(x in k.lower() for x in ['company','employer','organization'])]
        start_keys = [k for k in row.keys() if any(x in k.lower() for x in ['start','from'])]
        end_keys = [k for k in row.keys() if any(x in k.lower() for x in ['end','to'])]
        desc_keys = [k for k in row.keys() if any(x in k.lower() for x in ['description','summary','details','description'])]

        if len(rows) > 1 and (title_keys or company_keys):
            exps = []
            for r in rows:
                title = get_any(r, title_keys) if title_keys else ''
                company = get_any(r, company_keys) if company_keys else ''
                start = get_any(r, start_keys) if start_keys else ''
                end = get_any(r, end_keys) if end_keys else ''
                desc = get_any(r, desc_keys) if desc_keys else ''
...
def merge_parsed_into_user(parsed, ud):
    """
    Merge parsed fields into st.session_state['user_data'] without overwriting fields the user has already filled.
    """
    ...
    if parsed.get('skills') and (not ud.get('skills') or len(ud.get('skills')) == 0):
...
def main():
    ...
    # Step 1: Personal Info (with LinkedIn export upload option)
    if st.session_state.current_step == 1:
        ...
        st.markdown("### Optional: Import from LinkedIn export")
        st.info("If you downloaded your LinkedIn data (Profile CSV inside a ZIP), upload it here to pre-fill fields.")
        import_choice = st.radio("", ["Fill manually", "Upload LinkedIn export (ZIP/CSV)"], index=0, horizontal=True)

        if import_choice != "Fill manually":
            uploaded = st.file_uploader("Upload LinkedIn Data Export (ZIP or CSV)", type=['zip', 'csv'])
            if uploaded is not None:
                with st.spinner("Parsing LinkedIn export..."):
                    parsed = parse_linkedin_export_file(uploaded)
                ...
                # rerun so the UI shows updated values
                st.rerun()
